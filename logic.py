...
class GameBase(object):
    ...
    def change_speed(self):
        self.ui_data.speed = int(DEFAULT_SPEED * 100 / (100 + self.ui_data.score))
    ...
    def init(self):
        self.status = 0
        self.ui_data.blocks = np.zeros((self.height, self.width), dtype=np.uint8)
    ...
    def test_test_conflict(self, val):
        self.status = 0
        self.ui_data.blocks = np.ones((self.height, self.width), dtype=np.uint8)
